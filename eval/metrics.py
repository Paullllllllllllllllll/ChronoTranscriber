...
class FormattingMetrics:
    ...
    # Per-page presence flags (1 if present on this page, 0 if absent;
    # summed when aggregated)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "page_marker_recall": round(
                self.hyp_page_marker_pages / self.ref_page_marker_pages, 4
            )
            if self.ref_page_marker_pages > 0
            else None,
            "page_marker_recall_percent": round(
                self.hyp_page_marker_pages / self.ref_page_marker_pages * 100, 2
            )
            if self.ref_page_marker_pages > 0
            else None,
            ...
            if self.ref_heading_pages > 0
            else None,
            "heading_recall_percent": round(
                self.hyp_heading_pages / self.ref_heading_pages * 100, 2
            )
            if self.ref_heading_pages > 0
            else None,
            "ref_bold_italic_count": self.ref_bold_italic_count,
            "hyp_bold_italic_count": self.hyp_bold_italic_count,
            "bold_italic_ratio": round(
                self.hyp_bold_italic_count / self.ref_bold_italic_count, 4
            )
            if self.ref_bold_italic_count > 0
            else None,
            "bold_italic_ratio_percent": round(
                self.hyp_bold_italic_count / self.ref_bold_italic_count * 100, 2
            )
            if self.ref_bold_italic_count > 0
            else None,
        }
...
class TranscriptionMetrics:
    ...
    formatting: FormattingMetrics | None = None
...
def compute_cer(
    ...
    if len(ref_chars) == 0:
        # If reference is empty, CER is 0 if hypothesis is also empty, else
        # undefined (use hyp length)
        if len(hyp_chars) == 0:
            return 0.0, 0, 0, 0
...
def extract_formatting_elements(text: str) -> tuple[list[str], list[str]]:
    ...
    markdown_elements = []
    for _pattern_name, pattern in MARKDOWN_PATTERNS.items():
...
def compute_metrics(
    ...
    if compute_formatting:
        formatting_metrics = compute_formatting_metrics(
...
def aggregate_metrics(metrics_list: list[TranscriptionMetrics]) -> TranscriptionMetrics:
    ...
    agg_content_cer = (
        total_content_char_dist / total_ref_content_chars
        if total_ref_content_chars > 0
        else 0.0
    )
    agg_content_wer = (
        total_content_word_dist / total_ref_content_words
        if total_ref_content_words > 0
        else 0.0
    )

    # Aggregate formatting metrics
    formatting_metrics_list = [
        m.formatting for m in metrics_list if m.formatting is not None
    ]
    agg_formatting = None

    if formatting_metrics_list:
        total_pm_dist = sum(f.page_marker_distance for f in formatting_metrics_list)
        total_ref_pm_chars = sum(
            f.ref_page_marker_chars for f in formatting_metrics_list
        )
        total_hyp_pm_chars = sum(
            f.hyp_page_marker_chars for f in formatting_metrics_list
        )
        total_ref_pm_count = sum(
            f.ref_page_marker_count for f in formatting_metrics_list
        )
        total_hyp_pm_count = sum(
            f.hyp_page_marker_count for f in formatting_metrics_list
        )
        ...
        agg_pm_cer = (
            total_pm_dist / total_ref_pm_chars if total_ref_pm_chars > 0 else 0.0
        )
        agg_md_cer = (
            total_md_dist / total_ref_md_chars if total_ref_md_chars > 0 else 0.0
        )

        total_ref_pm_pages = sum(
            f.ref_page_marker_pages for f in formatting_metrics_list
        )
        total_hyp_pm_pages = sum(
            f.hyp_page_marker_pages for f in formatting_metrics_list
        )
...
def format_metrics_table(
    ...
    if categories is None:
        ...
        for cat_metrics in model_metrics.values():
            categories_set.update(cat_metrics.keys())
    ...
    if include_formatting:
        ...
        fmt_lines.append(
            "| Model | Category | Page Tag Recall (%) | Heading Recall (%) "
            "| Bold/Italic Ratio (%) |"
        )
        fmt_lines.append(
            "|-------|----------|---------------------|--------------------|-----------------------|"
        ...
        for model_name in sorted(model_metrics.keys()):
            for category in categories:
                if category in model_metrics[model_name]:
                    ...
                    if f:
                        pm = (
                            f"{f.hyp_page_marker_pages / f.ref_page_marker_pages * 100:.1f}"  # noqa: E501
                            if f.ref_page_marker_pages > 0
                            else "N/A"
                        )
                        hd = (
                            f"{f.hyp_heading_pages / f.ref_heading_pages * 100:.1f}"
                            if f.ref_heading_pages > 0
                            else "N/A"
                        )
                        bi = (
                            f"{f.hyp_bold_italic_count / f.ref_bold_italic_count * 100:.1f}"  # noqa: E501
                            if f.ref_bold_italic_count > 0
                            else "N/A"
                        )
                        fmt_lines.append(
                            f"| {model_name} | {category} | {pm} | {hd} | {bi} |"
                        )
                    else:
                        fmt_lines.append(
                            f"| {model_name} | {category} | N/A | N/A | N/A |"
                        )
