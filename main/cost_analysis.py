...
class CostAnalysisScript(DualModeScript):
    ...
    def run_interactive(self) -> None:
        ...
        self.logger.info("Starting cost analysis (Interactive Mode)")
        
        # Use file_paths as schemas_paths (PDFs and Images)
        schemas_paths = self.paths_config.get("file_paths", {})
        
        # Find .jsonl files
        print_info("Scanning for temporary .jsonl files...")
        jsonl_files = find_jsonl_files(self.paths_config, schemas_paths)
        
        if not jsonl_files:
            print_warning("No temporary .jsonl files found.")
            self.logger.warning("No .jsonl files found for analysis")
            return
    ...
    def run_cli(self, args: Namespace) -> None:
        ...
        self.logger.info("Starting cost analysis (CLI Mode)")
