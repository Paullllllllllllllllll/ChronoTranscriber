...
def check_exit(user_input: str) -> None:
    """
    Exit the program if the user input is 'q' or 'exit'.
    """
...
def safe_input(prompt: str) -> str:
    """
    Prompt the user for input safely.
...
async def transcribe_single_image_task(
    ...
    """
    Transcribe a single image using the specified method.

    Returns:
        Tuple[str, str, Optional[str]]: (image path, image name, transcription text)
    """
...
async def process_images_for_pdf(
    ...
    """
    Extract and preprocess images from a PDF.
...
async def process_single_pdf(
    ...
    """
    Process a single PDF file based on the selected transcription method.
    For native text extraction, only the output text file and JSONL record are created.
    For OCR or GPT-4o methods, additional image processing folders are
    created and subsequently processed.
    """
    ...
    if chosen_method is None:
        ...
        if choice not in valid_methods:
            console_print(
                f"[WARN] Invalid chosen method '{choice}' for PDF: {pdf_path.name}. Defaulting to the first available option.")
    ...
    # For non-native extraction methods, create the image processing folders
...
async def process_single_image_folder(
    ...
    """
    Process a single folder of images based on the selected transcription method.
    """
...
async def main() -> None:
    """
    Main entry point for the unified transcriber.
    """
    api_key = os.getenv('OPENAI_API_KEY')
    if not api_key:
        logger.error("Environment variable 'OPENAI_API_KEY' is not set.")
        console_print("[ERROR] OPENAI_API_KEY is not set. Please set it before running.")
        sys.exit(1)
    ...
    async with open_transcriber(
        ...
        choice = safe_input("Enter 1 for Images, 2 for PDFs (or type 'q' to exit): ")
        check_exit(choice)
        if choice == "1":
