...
class DailyTokenTracker:
    ...
    def would_block_next_page(self) -> bool:
        """True when the remaining budget cannot cover the current per-page
        reservation estimate — i.e. :meth:`try_reserve` would defer the next
        page even though :meth:`is_limit_reached` is still False.

        Admission control blocks a page when its reservation estimate (the
        rolling EWMA of observed per-call usage) exceeds the remaining budget,
        so near the cap pages are deferred while actual usage is still just
    ...
    def estimate_exceeds_daily_limit(self) -> bool:
        """True when the current per-page reservation estimate alone exceeds the
        entire daily limit, so even a full daily reset cannot admit the next
        page.
...
async def check_and_wait_for_token_limit(
    ...
    Returns:
        ...
        per-page estimate exceeds the entire daily limit, so no reset frees
        enough budget. Callers must treat False as an honest give-up (mark the
        item failed), never as success.
    """
    ...
    # Fast-fail: if a single page's estimate exceeds the whole daily limit, a
    # reset cannot admit it — waiting would burn ~48 h (two useless resets)
    # before the caller's stalled-resets safeguard fires. Give up now (CT-11).
    if reservation_aware and token_tracker.estimate_exceeds_daily_limit():
        ...
        logger.warning(
            "Per-page token estimate exceeds the entire daily limit (%s); "
            "a daily reset cannot admit the next page. Not waiting.",
            f"{daily_limit:,}",
        )
        print_warning(
            "A single page's token estimate exceeds the entire daily budget; "
            "not waiting. Raise daily_tokens to process the remaining pages."
        )
        return False
    ...
    try:
        ...
        if _still_blocked():
            logger.warning(
                ...
                "(per-page estimate exceeds the daily limit). Giving up."
            )
            print_warning(
                "Token limit reset elapsed but the next page still cannot be "
                "admitted; giving up. Raise daily_tokens to continue."
            )
            return False
