...
class TranscriptionOutput(BaseModel):
    ...
    image_analysis: str = Field(
        description=(
            ...
            "Identify the position of page number(s) and footnote(s) on the"
            " image precisely."
        )
    )
    ...
    transcription: str | None = Field(
        default=None,
        description=(
            "- Task: Return a **markdown-converted, verbatim transcription** of"
            " the provided image.\n"
            "- Scope — include everything: Transcribe **all** visible text and"
            " structural elements: "
            ...
            "- Layout preservation: **Preserve all line breaks** exactly as in the"
            " image.\n"
            "- Multi-column text: Transcribe column by column (finish column 1 before"
            " column 2, etc.), "
            "keep line breaks within columns intact.\n"
            "- Cross-page continuity: This is likely one page of a multi-page"
            " document. "
            "**Preserve any sentences that continue from the previous or to the next"
            " page exactly as shown.**\n"
            "- Output format: Provide **only** the transcription. If **no"
            " transcribable text** is found, output `null`.\n"
            "- No fabrication: **Do not hallucinate.** Keep spelling, punctuation,"
    ...
    no_transcribable_text: bool = Field(
        default=False,
        description=(
            "True if the image contains no transcribable text; otherwise, false."
        ),
    )
    ...
    transcription_not_possible: bool = Field(
        default=False,
        description=(
            "True *if and only if* transcription is entirely impossible;"
            " otherwise, false."
        ),
    )
...
def json_schema_to_pydantic_compatible(schema: dict[str, Any]) -> dict[str, Any]:
    """Convert a JSON schema to a format compatible with LangChain's
