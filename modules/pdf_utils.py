...
class PDFProcessor:
	...
	def prepare_output_folder(self, pdf_output_dir: Path) -> Tuple[
		Path, Path, Path]:
		"""
	...
	async def process_images(self, raw_images_folder: Path,
	                         preprocessed_folder: Path, target_dpi: int) -> \
	List[Path]:
		"""
		...
		"""
		try:
			await self.extract_images(raw_images_folder, dpi=target_dpi)
		except Exception as e:
			logger.exception(
				f"Error extracting images from PDF {self.pdf_path.name}: {e}")
			from modules.utils import console_print
			console_print(
				f"[ERROR] Failed to extract images from {self.pdf_path.name}.")
			return []
		# No need to import SUPPORTED_IMAGE_EXTENSIONS here since it's defined in image_utils.py
		from modules.image_utils import process_images_multiprocessing, \
		...
		if not image_files:
			from modules.utils import console_print
			console_print(
				f"[WARN] No images extracted from {self.pdf_path.name}.")
			return []
