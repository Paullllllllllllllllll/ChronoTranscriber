...
from modules.llm.transcriber import open_transcriber
...
class TestLangChainTranscriberIntegration:
    ...
    @pytest.mark.asyncio
    async def test_transcriber_initialization(self, mock_env_with_openai_key) -> None:
        """Test LangChainTranscriber can be initialized."""
        try:
            with patch(
                "modules.llm.providers.factory.get_provider"
            ...
            if (
                "CapabilityError" in str(type(e).__name__)
                or "Config" in str(e)
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_transcriber_processes_image(
        self, temp_dir: Path, mock_env_with_openai_key
    ) -> None:
        ...
        try:
            ...
            with patch(
                ...
                async with open_transcriber(
                    ...
                    transcriber.transcribe_image = AsyncMock(
                        return_value={
                            "transcription": "Transcribed content",
                            "no_transcribable_text": False,
                            "transcription_not_possible": False,
...
class TestBatchBackendIntegration:
    ...
    @pytest.mark.integration
    def test_openai_backend_initialization(self, mock_env_with_openai_key) -> None:
        """Test OpenAI batch backend can be initialized."""
        from modules.batch.backends.factory import get_batch_backend

        backend = get_batch_backend("openai")
        assert backend is not None

    @pytest.mark.integration
    def test_batch_request_creation(self, temp_dir: Path) -> None:
...
class TestProviderIntegration:
    ...
    @pytest.mark.integration
    def test_provider_factory(self, mock_env_with_openai_key) -> None:
        ...
        try:
            ...
            if (
                "CapabilityError" in str(type(e).__name__)
                or "Config" in str(e)
                or "ImportError" in str(type(e).__name__)
            ):
                pytest.skip(f"Test environment issue: {e}")
            raise
    ...
    def test_all_providers_importable(self) -> None:
        ...
        try:
            ...
            from modules.llm.providers.factory import ProviderType, get_provider
...
class TestSchemaIntegration:
    ...
    @pytest.mark.integration
    def test_schema_loading_and_validation(self) -> None:
...
class TestErrorHandlingIntegration:
    ...
    @pytest.mark.integration
    def test_graceful_config_error_handling(self) -> None:
    ...
    @pytest.mark.integration
    def test_invalid_json_handling(self, temp_dir: Path) -> None:
