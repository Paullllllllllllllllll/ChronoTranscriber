...
class TestWorkflowManagerIntegration:
    ...
    def test_manager_initialization(self, workflow_manager) -> None:
    ...
    @pytest.mark.integration
    def test_output_directory_creation(self, workflow_manager, temp_dir: Path) -> None:
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_tesseract_not_available_handling(self, workflow_manager) -> None:
        """Test handling when Tesseract is not available."""
        with patch.object(
            workflow_manager, "_ensure_tesseract_available", return_value=False
        ):
...
class TestPostProcessingIntegration:
    ...
    @pytest.mark.integration
    def test_full_postprocessing_pipeline(self, temp_dir: Path) -> None:
        """Test the complete post-processing pipeline on a file."""
        from modules.postprocess.text import postprocess_file
        ...
        result_path = postprocess_file(
            input_file, output_path=output_file, config=config
        )
...
class TestConfigIntegration:
    ...
    @pytest.mark.integration
    def test_config_service_loads_all_configs(self) -> None:
        ...
        try:
            ...
            # These should not raise (but may fail in test env due to capability checks)
            try:
                ...
                if "CapabilityError" in str(type(e).__name__):
                    pytest.skip(
                        f"Config loading failed due to model capability check: {e}"
                    )
                raise
        finally:
            ConfigService.reset()
...
class TestAutoSelectorIntegration:
    ...
    @pytest.mark.integration
    def test_auto_selector_scans_mixed_directory(
        self, temp_dir: Path, mock_paths_config: dict[str, Any], mock_env_no_api_keys
    ) -> None:
    ...
    @pytest.mark.integration
    def test_auto_selector_creates_decisions(
        self, temp_dir: Path, mock_paths_config: dict[str, Any], mock_env_no_api_keys
    ) -> None:
...
class TestCliIntegration:
    ...
    @pytest.mark.integration
    def test_transcriber_parser_full_args(self) -> None:
    ...
    @pytest.mark.integration
    def test_repair_parser_all_flags(self) -> None:
...
class TestTextProcessingIntegration:
    ...
    @pytest.mark.integration
    def test_extract_from_various_response_formats(self) -> None:
        ...
        for fmt, exp in zip(formats, expected, strict=False):
            result = extract_transcribed_text(fmt)
            assert result == exp

    @pytest.mark.integration
    def test_batch_output_processing(self) -> None:
        """Test processing batch output with multiple records."""
        from modules.llm.response_parsing import process_batch_output
