...
class TestConfigureAutoModeSchema:
    ...
    def test_skips_when_decisions_are_non_gpt(self) -> None:
        """Schema selection is skipped when all decisions use non-GPT methods."""
    ...
    def test_prompts_when_gpt_decisions_exist(self) -> None:
        ...
        with (
            patch.object(
                WorkflowUI, "configure_schema_selection", return_value=True
            ) as mock_schema,
            patch.object(
    ...
    def test_returns_false_when_context_selection_cancelled(self) -> None:
        ...
        with (
            ...
            patch.object(
                WorkflowUI, "configure_additional_context", return_value=False
            ),
        ):
            result = WorkflowUI.configure_auto_mode_schema(config)
...
class TestAutoModeCliSchemaSelection:
    ...
    @pytest.mark.unit
    def test_auto_mode_with_schema_argument(
    ...
    @pytest.mark.unit
    def test_auto_mode_invalid_schema_raises_error(
        self, temp_dir: Path, mock_paths_config: dict[str, Any]
    ) -> None:
    ...
    @pytest.mark.unit
    def test_auto_mode_default_schema_when_not_specified(
        self, temp_dir: Path, mock_paths_config: dict[str, Any]
    ) -> None:
...
class TestAutoModeContextSelection:
    ...
    @pytest.mark.unit
    def test_auto_mode_with_context_argument(
        self, temp_dir: Path, mock_paths_config: dict[str, Any]
    ) -> None:
