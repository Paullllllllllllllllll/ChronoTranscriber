...
class TestProgressStateInit:
    ...
    def test_auto_sets_start_time(self) -> None:
...
class TestProgressStateProperties:
    ...
    def test_elapsed_seconds_none_start(self) -> None:
        state = ProgressState(total=10)
        state.start_time = None
        assert state.elapsed_seconds == 0.0

    def test_estimated_remaining_no_progress(self) -> None:
        state = ProgressState(total=10)
        assert state.estimated_remaining_seconds is None

    def test_estimated_remaining_some_progress(self) -> None:
        past = datetime.now() - timedelta(seconds=10)
    ...
    def test_estimated_remaining_all_done(self) -> None:
        state = ProgressState(total=5, completed=5)
        assert state.estimated_remaining_seconds is None
...
class TestProgressStateFormatSummary:
    ...
    def test_format_summary_with_estimate(self) -> None:
        past = datetime.now() - timedelta(seconds=10)
        state = ProgressState(total=10, completed=5, start_time=past)
        summary = state.format_summary()
...
class TestProgressTracker:
    @pytest.mark.asyncio
    async def test_increment_completed(self) -> None:
        tracker = ProgressTracker(total=10)
        await tracker.increment_completed()
        assert tracker.state.completed == 1

    @pytest.mark.asyncio
    async def test_increment_failed(self) -> None:
        tracker = ProgressTracker(total=10)
        await tracker.increment_failed()
        assert tracker.state.failed == 1

    @pytest.mark.asyncio
    async def test_callback_called_at_interval(self) -> None:
        callback = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_callback_called_at_completion(self) -> None:
        callback = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_finalize_calls_callback(self) -> None:
        callback = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_finalize_without_callback(self) -> None:
        tracker = ProgressTracker(total=10)
        await tracker.finalize()  # Should not raise

    @pytest.mark.asyncio
    async def test_finalize_callback_error_does_not_crash(self) -> None:
        def bad_callback(state):
